...
#define	__FIFO_PEEK(data, out, mask) \
	((data)[(out) & (mask)])
/*
...
static unsigned int __fifo_peek_n(struct __fifo *fifo, size_t recsize)
{
	...
	l = __FIFO_PEEK(data, fifo->out, mask);

	if (--recsize)
		l |= __FIFO_PEEK(data, fifo->out + 1, mask) << 8;

	return l;
}

#define	__FIFO_POKE(data, in, mask, val) \
	( \
...
static void __fifo_poke_n(struct __fifo *fifo, unsigned int n, size_t recsize)
{
	...
	__FIFO_POKE(data, fifo->in, mask, n);

	if (recsize > 1)
		__FIFO_POKE(data, fifo->in + 1, mask, n >> 8);
}
